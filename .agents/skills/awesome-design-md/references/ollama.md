...
- Contains the literal `[network install command omitted]` install command with a small copy-icon at the right edge. Sits directly below the hero headline as the page's most prominent "CTA."
...
- Validate token references and contrast using only checks already authorized by the project.
